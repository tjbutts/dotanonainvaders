...
# Base GAM
phyto_base_gam <- gam(biomass_mgL ~ s(doy, k = 40, bs = 'cs') , Gamma(link = 'log'), data = phyto_base, method = 'REML') # Cubic Regression Spline
summary(phyto_base_gam) 
gam.check(phyto_base_gam)

phyto_SWF_gam <- gam(biomass_mgL ~ s(doy, k = 40, bs = 'cs') , Gamma(link = 'log'), data = phyto_SWF, method = 'REML') # Cubic Regression Spline
summary(phyto_SWF_gam) 
gam.check(phyto_SWF_gam)

phyto_ZM_gam <- gam(biomass_mgL ~ s(doy, k = 40, bs = 'cs') , Gamma(link = 'log'), data = phyto_ZM, method = 'REML') # Cubic Regression Spline
